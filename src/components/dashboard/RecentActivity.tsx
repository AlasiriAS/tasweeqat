...
export async function RecentActivity() {
  ...
  return (
    <div className="grid lg:grid-cols-2 gap-6">
      ...
      <div className="bg-white dark:bg-[#0f2419] rounded-2xl p-6 border border-gray-100 dark:border-white/8 shadow-sm">
          ...
          <div className="space-y-3">
            {activities.map((a) => (
              <div key={a.id} className="flex items-start gap-3 py-2 border-b border-gray-50 dark:border-white/5 last:border-0">
                <div className="w-8 h-8 bg-[#1B5E4B]/10 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5">
                  <span className="text-[#2E7D68] text-xs">●</span>
                </div>
      ...
      <div className="bg-white dark:bg-[#0f2419] rounded-2xl p-6 border border-gray-100 dark:border-white/8 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          ...
          {contacts.length > 0 && (
            <span className="bg-[#E6C16A] text-[#3a2c0c] text-xs font-bold px-2.5 py-1 rounded-full">
              {contacts.length} new
            </span>
          )}
        </div>
          ...
          <div className="space-y-3">
            {contacts.map((c) => (
              <div key={c.id} className="flex items-start gap-3 py-2 border-b border-gray-50 dark:border-white/5 last:border-0">
                <div className="w-9 h-9 bg-[#1B5E4B] rounded-lg flex items-center justify-center flex-shrink-0 text-white text-sm font-bold">
                ...
                <span className="text-[#2E7D68] text-xs flex-shrink-0 bg-[#1B5E4B]/10 px-2 py-0.5 rounded-full">
                  new
                </span>
              </div>
            ))}
