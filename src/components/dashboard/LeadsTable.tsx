...
export function LeadsTable({ leads, cities, filters }: Props) {
  ...
  return (
    <div className="bg-white dark:bg-[#0f2419] rounded-2xl border border-gray-100 dark:border-white/8 shadow-sm overflow-hidden">
      {/* Filters bar */}
      <div className="p-4 border-b border-gray-100 dark:border-white/8 flex flex-wrap gap-3 items-center">
              ...
              <tr key={lead.id} className="hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                ...
                <td className="px-4 py-3">
                  {lead.rating ? (
                    <div className="flex items-center gap-1">
                      <Star size={12} className="text-[#c9a24e] fill-[#E6C16A]" />
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    {lead.phone && (
                      <a href={`tel:${lead.phone}`} className="text-[#2E7D68] hover:text-green-600">
                        <Phone size={14} />
                      </a>
                    )}
                    {lead.website && (
                      <a href={lead.website} target="_blank" rel="noopener noreferrer" className="text-[#1B5E4B] dark:text-[#E6C16A] hover:opacity-80">
                        <Globe size={14} />
                      </a>
                    )}
                  </div>
                ...
                <td className="px-4 py-3">
                  {lead.pipelineStage ? (
                    <span className="text-[#2E7D68] text-xs font-semibold">✓ In Pipeline</span>
                  ) : (
                    <button
                      onClick={() => addToPipeline(lead.id)}
                      disabled={adding === lead.id}
                      className="flex items-center gap-1 text-xs bg-[#1B5E4B]/10 hover:bg-[#1B5E4B] text-[#2E7D68] hover:text-white px-3 py-1.5 rounded-lg font-semibold transition-all"
                    >
                      <Plus size={12} />
